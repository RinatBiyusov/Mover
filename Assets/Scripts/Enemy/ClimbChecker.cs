...
public class ClimbChecker : MonoBehaviour
{
    ...
    {
        Vector3 ceilingRay = new Vector3(_collider.bounds.center.x, _collider.bounds.max.y, _collider.bounds.center.z);
        Vector3 floorRay = new Vector3(_collider.bounds.center.x, _collider.bounds.min.y, _collider.bounds.center.z);

        if (Physics.Raycast(ceilingRay, transform.forward, _gapDistance, _layerMask))
        {
